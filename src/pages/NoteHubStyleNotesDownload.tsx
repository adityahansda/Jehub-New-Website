...
import { Search, Filter, Download, Eye, Calendar, User, Tag, CheckCircle, X, Heart, Share2, Grid, List, FileText, Coins, AlertTriangle, Menu, Home, BookOpen, Upload, Users, ChevronDown, Star, MapPin, Building, GraduationCap, Clock, BookmarkPlus } from 'lucide-react';
...
const NoteHubStyleNotesDownload = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-40 pt-safe-area-inset-top">
        {/* Mobile safe area spacing */}
        <div className="h-2 sm:h-0 bg-white"></div>
      ...
      <div className="max-w-7xl mx-auto flex">
        {/* Sidebar - Modern Study Hub Style */}
        <aside className={(sidebarOpen ? 'translate-x-0' : '-translate-x-full') + ' lg:translate-x-0 fixed lg:sticky top-20 sm:top-16 left-0 z-30 w-64 h-screen lg:h-auto bg-white border-r border-gray-200 transition-transform duration-300 ease-in-out lg:transition-none overflow-y-auto'}>
          ...
          <div className="p-4">
              ...
              {sidebarItems.map((item, index) => (
                <div key={index}>
                  {item.href ? (
                    <Link
                      href={item.href}
                      className={'flex items-center justify-between p-2 rounded-lg text-sm transition-colors ' + (item.isActive ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-700 hover:bg-gray-50')}
                    >
                      <div className="flex items-center space-x-3">
                    ...
                    <button
                      ...
                      className={'w-full flex items-center justify-between p-2 rounded-lg text-sm transition-colors ' + (item.isActive ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-700 hover:bg-gray-50')}
                    >
                      <div className="flex items-center space-x-3">
                        <item.icon className="h-4 w-4" />
                        <span>{item.label}</span>
                ...
                {courseFilters.map((course, index) => (
                  <button
                    ...
                    onClick={() => setSelectedFilters(prev => ({
                      ...prev,
                      branch: prev.branch === course.value ? '' : course.value
                    }))}
                    className={'w-full flex items-center justify-between p-2 rounded-lg text-sm transition-colors ' + (selectedFilters.branch === course.value ? 'bg-blue-50 text-blue-600 border border-blue-200' : 'text-gray-600 hover:bg-gray-50 border border-transparent')}
                  >
                    <span className="flex items-center gap-2">
                      <div className={'w-2 h-2 rounded-full ' + (selectedFilters.branch === course.value ? 'bg-blue-500' : 'bg-gray-300')}></div>
                      {course.label}
                    </span>
                    <span className={'text-xs px-2 py-1 rounded-full ' + (selectedFilters.branch === course.value ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600')}>
                      {course.count}
                    </span>
                  </button>
                ))}
            ...
            <div className="mb-6">
              ...
              <div className="space-y-3">
                ...
                {/* Degree Filter */}
                <div>
                  <label className="text-xs text-gray-600 mb-1 block">Degree</label>
                  <select
                ...
                {Object.values(selectedFilters).some(v => v) && (
                  <button
                    onClick={() => setSelectedFilters({ branch: '', semester: '', subject: '', degree: '', noteType: '' })}
        ...
        <main className="flex-1 p-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
            <h1 className="text-2xl font-bold text-gray-900 mb-4 sm:mb-0">Notes Download</h1>
          </div>
